function [] = DefineEdge(path, sigma, px_to_micro)
%%%% AIM %%%%
...
if ~exist([path,filesep,'Matlab_data'])
    mkdir([path, filesep,'Matlab_data']) ;
end

img = imadjust(mat2gray(imread([path,filesep,'CAF.tif']))) ;
    

%% Define the CAF edge   (from Gerardo's script)
...
imshow(img) ; ;;
title(sprintf('Delineate the inside part of the spheroid \n Click to draw the polygon/Move the points \n A+click to add points \n Double click to finish'))
Mask=roipoly; % to draw, use the roipoly function from MATLAB
close(fig) ; 
 
Mask=double(Mask); % save the mask as a double object
Mask = imbinarize(imgaussfilt(Mask,sigma)) ;   %smoothing of the mask
