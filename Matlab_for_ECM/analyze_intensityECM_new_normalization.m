...
%normalize the intensities to the OUT region intensity 
%%%%%%%%%%%%%%% DEFINE THE OUT REGION HERE %%%%%%%%%%%%%%
baseline = mean(data(find(Radius >= 120*2 & Radius <= 120*3))) ; % we take the mean intensity of the image within the out region
for k  = 1 : length(data_vec)  
    data_vec{k} = data_vec{k}/baseline; 
end 
